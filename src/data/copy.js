...
export const COPY = {
  en: {
    ...
    faq: [
      ...
      ["How is my journal protected?", "The journal is encrypted at rest using AES-256 with optional PIN or device biometric support."],
...
COPY.ja = {
  ...
  features: { eyebrow: "/ アプリ全体", h2: "フィードではなく、ノートのように。", p: "連続記録の罪悪感も、友達申請も、余計な機能もなし。実際に使う日記の機能だけ。", cards: [
    ...
    { tag: "06 · 記録追加", h3: "文章以上を残す。", p: "写真、音声メモ、小さなステッカーを記録に追加。すべて暗号化された日記ファイル内に保存されます。", bullets: ["写真・画像添付", "日記用の音声メモ", "ステッカーパック", "暗号化された .gamu ファイル内に保存"] },
  ...
  faq: [["Gamu Journal にアカウントは必要ですか？", "いいえ。登録、メール、ユーザー名はありません。インストールしてすぐ書き始められます。ユーザーデータベースもありません。"], ["日記はクラウド同期されますか？", "Gamu Journal にクラウド同期はありません。記録は書いた端末に残ります。別の場所に保管したい場合は、暗号化バックアップファイルを書き出して、信頼する場所に保存できます。"], ["日記はどう守られますか？", "日記はAES-256で保存時に暗号化されます。PINや端末の生体認証は任意で設定できます。"], ["スマホをなくしたらどうなりますか？", "バックアップを書き出していなければ、記録は端末と一緒に失われます。復元用サーバーはありません。暗号化された <code>.gamu</code> ファイルを定期的に信頼する場所へ保存してください。"], ["サブスクリプションですか？", "いいえ。無料版は無料のまま、Pro はアプリストア経由の買い切りです。更新も、自動課金される無料体験も、Pro+ もありません。"], ["iPhone版はありますか？", "はい。Gamu Journal は App Store で入手できます。バックアップを書き出さない限り、日記は書いた端末に残ります。"]],
...
COPY.pt_BR = {
  ...
  features: { eyebrow: "/ O app inteiro", h2: "Feito como caderno, não como feed.", p: "Sem culpa por sequência, sem pedidos de amizade, sem recursos que você não pediu. Só as partes de um diário que você realmente usa.", cards: [
    ...
    { tag: "06 · Captura", h3: "Capture mais que texto.", p: "Anexe uma foto, grave uma nota de voz ou adicione um adesivo. Tudo fica dentro do arquivo criptografado do diário.", bullets: ["Fotos e imagens anexadas", "Notas de voz para suas entradas", "Pacote de adesivos", "Guardado no arquivo .gamu criptografado"] },
...
const compactLocales = {
  ru: {
    ...
    faq: [["Нужен ли аккаунт?", "Нет. Нет регистрации, email или имени пользователя. Установите приложение и начинайте писать."], ["Дневник синхронизируется с облаком?", "Нет. Встроенной облачной синхронизации нет. Записи остаются на устройстве, где они были написаны."], ["Как защищен дневник?", "Дневник зашифрован AES-256 в покое с опциональной поддержкой PIN или биометрии устройства."], ["Что если я потеряю телефон?", "Без экспортированной резервной копии записи потеряются вместе с устройством. Серверов восстановления нет. Периодически сохраняйте зашифрованный <code>.gamu</code>."], ["Есть подписка?", "Нет. Бесплатная версия остается бесплатной, Pro, разовая покупка через магазины."], ["Есть версия для iPhone?", "Да. Gamu Journal доступен в App Store."]],
  ...
  es: {
    ...
    privacyCards: [["01 · Sin cuenta", "No te registras.", "Sin email, usuario ni Entrar con Google. Abres la app. Escribes. Eso es todo."], ["02 · Sin nube", "Tus entradas no salen del teléfono.", "No operamos un servidor para tu diario. Los datos viven en el almacenamiento cifrado de la app."], ["03 · Protección local", "PIN, contraseña o biometría.", "El diario está cifrado en reposo y se desbloquea solo en este dispositivo."], ["04 · Tus copias", "Un archivo, no un servicio.", "Exporta un <code>.gamu</code> cifrado y guárdalo donde quieras."]],
    ...
    faq: [["¿Gamu Journal necesita una cuenta?", "No. No hay registro, email ni usuario. Instalas la app y empiezas a escribir."], ["¿Mi diario se sincroniza con la nube?", "No. Gamu Journal no tiene sincronización en la nube. Tus entradas se quedan en el dispositivo donde las escribes."], ["¿Cómo se protege mi diario?", "El diario está cifrado en reposo con AES-256, con soporte opcional de PIN o biometría del dispositivo."], ["¿Qué pasa si pierdo el teléfono?", "Si no exportaste una copia, tus entradas se pierden con el dispositivo. No hay servidor para restaurarlas. Exporta periódicamente un <code>.gamu</code> cifrado."], ["¿Hay suscripción?", "No. Gratis sigue siendo gratis y Pro es un pago único en las tiendas."], ["¿Hay versión para iPhone?", "Sí. Gamu Journal está disponible en la App Store."]],
  ...
  ko: {
    ...
    faq: [["Gamu Journal에 계정이 필요한가요?", "아니요. 가입, 이메일, 사용자 이름이 없습니다. 앱을 설치하고 바로 쓰면 됩니다."], ["일기가 클라우드에 동기화되나요?", "아니요. Gamu Journal에는 클라우드 동기화가 없습니다. 기록은 작성한 기기에 남습니다."], ["일기는 어떻게 보호되나요?", "일기는 AES-256으로 저장 시 암호화되며 PIN 또는 기기 생체 인증을 선택적으로 사용할 수 있습니다."], ["휴대폰을 잃어버리면 어떻게 되나요?", "백업을 내보내지 않았다면 기록은 기기와 함께 사라집니다. 복원 서버는 없습니다. 암호화된 <code>.gamu</code> 파일을 주기적으로 저장하세요."], ["구독인가요?", "아니요. 무료는 계속 무료이고 Pro는 스토어를 통한 1회 구매입니다."], ["iPhone 버전이 있나요?", "네. Gamu Journal은 App Store에서 사용할 수 있습니다."]],
